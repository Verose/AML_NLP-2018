...
class NERModel(Model):
    ...
    def consolidate_predictions(self, data_raw, data, preds, probs):
        """
    ...
    def evaluate(self, sess, examples, examples_raw, summarize=False):
        ...
        for data  in self.output(sess, examples_raw, examples, summarize):
            if summarize:
                (_, labels, labels_, probs_) = data
            else:
    ...
    def output(self, sess, inputs_raw, inputs=None, summarize=False):
        ...
        for i, batch in enumerate(minibatches(inputs, self.config.batch_size, shuffle=False)):
            ...
            if summarize:
                preds_, probs_ = self.predict_on_batch(sess, *batch, summarize=True)
                preds += list(preds_)
                probs += list(probs_)
            else:
        ...
        for epoch in range(self.config.n_epochs):
            ...
            for i, minibatch in enumerate(minibatches(train_examples, self.config.batch_size)):
                ...
                if writer is None:
                    ...
                    loss, summary = self.train_on_batch(sess, *data, summarize=True)
