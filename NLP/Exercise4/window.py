...
logger = logging.getLogger("hw4.q1")
        ...
        for index, (word, label) in enumerate(zip(sentence, labels)):
            ...
            while i <= index + window_size:
                if i < 0:
                    ...
                    window.extend(sentence[i])
                i += 1
            feature = (window, label)
            windowed_data += [feature]
        # END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(tf.int32, (None, self.config.n_window_features), "input")
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(index_into_embeds_tens, [-1, self.config.n_window_features * self.config.embed_size])
        # END YOUR CODE
        return embeddings
    ...
    def consolidate_predictions(self, examples_raw, examples, preds, probs=None):
    ...
    def predict_on_batch(self, sess, inputs_batch, summarize=False):
    ...
    def train_on_batch(self, sess, inputs_batch, labels_batch, summarize=False):
